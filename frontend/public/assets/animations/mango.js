...
window.startMango = (mountEl) => {
  ...
  function unifyMaterials(root, { env = 1.2, rough = 0.6, metal = 0.0 } = {}) {
    root.traverse((o) => {
      if (o.isMesh) {
        o.castShadow = true
  ...
  function init() {
    ...
    params = { mangoSpin: true, mangoSpeed: 0.05, cameraSpin: false, cameraSpeed: 0.01, autorotate: true, autorotateSpeed: 0.2 }

    const loader = new GLTFLoader()
    loader.load('./img/mango-obj/scene.gltf', (gltf) => {
      gltf.scene.updateMatrixWorld(true)
      gltf.scene.traverse((c) => { if (c.isMesh && !mango) { mango = c; mango.castShadow = true; mango.receiveShadow = true } })
      if (!mango) return
      ...
      gltf.scene.traverse((c) => {
        if (c.isMesh && c.material) {
          const m = c.material
          ;['map', 'normalMap', 'roughnessMap', 'metalnessMap', 'aoMap', 'emissiveMap'].forEach(k => { if (m[k]) m[k].anisotropy = maxAniso })
        }
      })
    ...
    asteroidLoader.load('./img/astroids/scene.gltf', (gltf) => {
      gltf.scene.updateMatrixWorld(true)
      const asteroids = []
      gltf.scene.traverse((c) => { if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; asteroids.push(c) } })
      if (!asteroids.length) return

      const positions = [new THREE.Vector3(-5, -2.54, -3.03), new THREE.Vector3(2.87, -0.08, -0.9), new THREE.Vector3(-7.95, 2.13, -3.28)]
  ...
  function makeTextRing(text = 'CREATIVE CODING • DIGITAL DESIGN • ') {
    ...
    function draw() {
      ...
      let x = -m
      const y = H / 2
      while (x < W + m) { ctx.fillText(phrase, x + PADX, y); x += m }
  ...
  function addStars() {
    ...
    for (let i = 0; i < sets; i++) {
      ...
      const starsMaterial = new THREE.PointsMaterial({ size: 4, transparent: true, blending: THREE.AdditiveBlending, depthTest: true, depthWrite: false, map: getTexture(i + 1) })
  ...
  function getTexture(type) {
    ...
    if (type === 1) { gradient.addColorStop(0, 'rgba(255,255,255,1)'); gradient.addColorStop(0.2, 'rgba(200,255,255,1)'); gradient.addColorStop(0.4, 'rgba(0,0,124,1)'); gradient.addColorStop(1, 'rgba(0,0,0,1)') }
    else if (type === 2) { gradient.addColorStop(0, 'rgba(255,255,255,1)'); gradient.addColorStop(0.2, 'rgba(241,220,202,1)'); gradient.addColorStop(0.4, 'rgba(239,120,23,1)'); gradient.addColorStop(1, 'rgba(0,0,0,1)') }
    else { gradient.addColorStop(0, 'rgba(255,255,255,1)'); gradient.addColorStop(0.2, 'rgba(255,255,255,1)'); gradient.addColorStop(0.4, 'rgba(192,247,216,1)'); gradient.addColorStop(1, 'rgba(0,0,0,1)') }
  ...
  function animate() {
    ...
    if (starsSphere) { starsSphere.rotation.x += 0.0004; starsSphere.rotation.y += 0.0004 }
    if (asteroidParents && asteroidParents.length) { asteroidParents.forEach((parent) => { parent.rotation.x += -0.006; parent.rotation.y += -0.006 }) }
  ...
  const CAM = { scrub: 0.45, ease: 'power3.out', smooth: 0.22, spins: Math.PI, r0: 1, r1: 1, roll0: 0, roll1: 0 }
    ...
    return function section(cfg) {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          if (cfg.posLerp) {
            camera.position.set(
              lerp(camera.position.x, x, cfg.posLerp),
              lerp(camera.position.y, y, cfg.posLerp),
              lerp(camera.position.z, z, cfg.posLerp)
            )
          } else {
          ...
          if (controls) {
            const s = cfg.targetLerp ?? CAM.smooth
            const cur = controls.target
            cur.set(lerp(cur.x, tx, s), lerp(cur.y, ty, s), lerp(cur.z, tz, s))
            controls.update()
          } else {
            camera.lookAt(tx, ty, tz)
          }

          camera.rotation.z = gsap.utils.interpolate(roll0, roll1, t)
  ...
  useSection({
    trigger: '.story-home',
    spins: Math.PI * 1.1,
    radiusStart: 1.0, radiusEnd: 1.1,
    yStart: camera.position.y, yEnd: camera.position.y - 0.2,
    rollStart: 0, rollEnd: 0.15,
    targetX0: 0, targetX1: 1,
    ease: 'expo.out',
    priority: 1
  })
  ...
  useSection({
    trigger: '.projects',
    spins: Math.PI * 0.65,
    radiusStart: 1.15, radiusEnd: 0.92,
    yStart: camera.position.y - 0.2, yEnd: camera.position.y + 0.15,
    rollStart: 0.05, rollEnd: -0.1,
    targetX0: -0.6, targetX1: 5,
    targetY0: 0.0,  targetY1: 0.2,
    targetZ0: 0.0,  targetZ1: -5,
    priority: 2
  })
  ...
  useSection({
    trigger: '.about-home',
    spins: Math.PI * 0.65,
    radiusStart: 0.92, radiusEnd: 0.5,
    yStart: camera.position.y - 0.2, yEnd: camera.position.y + 0.15,
    rollStart: -0.1, rollEnd: -0.3,
    targetX0: 5, targetX1: 10,
    targetY0: 0.2, targetY1: 2,
    targetZ0: -5, targetZ1: 20,
    priority: 3
  })
  ...
  useSection({
    trigger: '.blogs',
    spins: Math.PI * 0.65,
    radiusStart: 0.5, radiusEnd: 0.5,
    yStart: camera.position.y + 0.15, yEnd: camera.position.y + 0.15,
    rollStart: -0.3, rollEnd: -0.3,
    targetX0: 10, targetX1: 15,
    targetY0: 2, targetY1: -2,
    targetZ0: 20, targetZ1: 10,
    priority: 4
  })
}
  ...
  function hideHeroText() {
    ...
    ScrollTrigger.create({
      ...
      scrub: true,
      onUpdate: (self) => { ringFade = self.progress }
    })
  }
